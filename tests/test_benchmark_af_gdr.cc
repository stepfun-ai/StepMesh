...
#ifdef STEPAF_USE_GDR
void InitGPUVals(std::vector<SArray<char>>& vals, int num, size_t len) {
    vals.resize(num);
    for (int i = 0; i < num; ++i) {
        void* ptr;
        cudaError_t e = cudaMalloc(&ptr, len);
        PS_CHECK_EQ(e, cudaSuccess) << "cudaMalloc failed, size=" << len;
        vals[i].reset(reinterpret_cast<char*>(ptr), len, [](void* p) {
            cudaError_t e = cudaFree(p);
            PS_CHECK_EQ(e, cudaSuccess);
        });
    }
}
#endif
...
#ifdef STEPAF_USE_GDR
        void* ptr_val;
        cudaError_t e = cudaMalloc(&ptr_val, len);
        PS_CHECK_EQ(e, cudaSuccess);
...
#ifdef STEPAF_USE_GDR
  PS_LOG(INFO) << "init work push buffer with GDR: microbatch="
            << g_conf.mb_num << ", val_size=" << af_push_len;
  InitGPUVals(g_af_push_vals, g_conf.mb_num, af_push_len);

  PS_LOG(INFO) << "init work pull buffer with GDR: microbatch="
            << g_conf.mb_num << ", val_size=" << af_pull_len;
  InitGPUVals(g_af_pull_vals, g_conf.mb_num, af_pull_len);
#else
